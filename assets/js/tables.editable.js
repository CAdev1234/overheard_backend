...
(function($) {
    ...
    let ReporterListTable = {

        options: {
            table: '#datatable-reporterlist',
            dialog: {
                wrapper: '#dialog',
                reset_password_wrapper: '#password-reset-dialog',
                cancelButton: '#dialogResetCancel',
        ...
        build: function() {
            ...
            this.datatable = this.$table.DataTable({
                ...
                sZeroRecords: "No Users Found",
                ajax: {
                    url: app_url + '/getReporterListData',
                    type: 'post',
                    data: {
                        _token: $('meta[name=csrf-token]').attr("content"),
                    }
                    ...
                    {data: 'email'},
                    {
                        ...
                        render: function (data, type, row, meta) {
                            if(data['status'] == 0){
                                ...
                                element += '<div class="col-xl-6" style="text-align: center">'
                                element += '<button type="button" class="mb-1 mt-1 mr-1 btn btn-success" reporter-id="' + data.id +  '"  onclick="approveUser($(this))">Approve</button>';
                                element += '</div>';
                                element += '<div class="col-xl-6" style="text-align: center">';
                                element += '<button type="button" class="mb-1 mt-1 mr-1 btn btn-danger" reporter-id="' + data.id +  '" onclick="declineUser($(this))">Decline</button>';
                                element += '</div></div>'
                                return element;
                            }
                                ...
                                element += '<button type="button" class="mb-1 mt-1 mr-1 btn btn-info">Approved</button>';
                                element += '</div>';
                                element += '</div';
                                return element;
                            }
                                ...
                                let element = '<div class="row" style="margin: 0px;">';
                                element += '<div class="col-xl-12" style="text-align: center">'
                                element += '<button type="button" class="mb-1 mt-1 mr-1 btn btn-warning">Declined</button>';
                                element += '</div>';
                                element += '</div';
                                return element;
                            }
    ...
    let CommunityListTable = {

        options: {
            table: '#datatable-communitylist',
            dialog: {
                wrapper: '#dialog',
                reset_password_wrapper: '#password-reset-dialog',
                cancelButton: '#dialogResetCancel',
        ...
        build: function() {
            ...
            this.datatable = this.$table.DataTable({
                ...
                sZeroRecords: "No Users Found",
                ajax: {
                    url: app_url + '/getCommunityListData',
                    type: 'post',
                    data: {
                        _token: $('meta[name=csrf-token]').attr("content"),
                    }
                ...
                aoColumns: [
                    ...
                    {data: 'created_at'},
                    {
                        ...
                        render: function (data, type, row, meta) {
                            let element = '<div class="row" style="margin: 0px;">';
                            element += '<div class="col-xl-12" style="text-align: center">'
                            element += '<button style="width: 100px;" type="button" class="view-row mb-1 mt-1 mr-1 btn btn-info">View</button>';
                            element += '</div>';
                            element += '</div';
                            return element;
                        }
        ...
        events: function() {
            ...
            this.$table
                .on( 'click', 'button.view-row', function( e ) {
                    e.preventDefault();

                    var $row = $(this).closest( 'tr' ),
                        userId = $row.attr('data-item-id');

                    window.location.href = app_url + '/community_detail/' + userId;
                });
            return this;
        }
    };

    let CommunitySubmitListTable = {

        options: {
            table: '#datatable-communitysubmitlist',
            dialog: {
                wrapper: '#dialog',
                reset_password_wrapper: '#password-reset-dialog',
                cancelButton: '#dialogResetCancel',
        ...
        build: function() {
            ...
            this.datatable = this.$table.DataTable({
                ...
                sZeroRecords: "No Users Found",
                ajax: {
                    url: app_url + '/getSubmittedCommunityListData',
                    type: 'post',
                    data: {
                        _token: $('meta[name=csrf-token]').attr("content"),
                    }
                ...
                aoColumns: [
                    ...
                    {data: 'created_at'},
                    {
                        ...
                        render: function (data, type, row, meta) {
                            let element = '<div class="row" style="margin: 0px;">';
                            element += '<div class="col-xl-6" style="text-align: center">'
                            element += '<button type="button" class="mb-1 mt-1 mr-1 btn btn-success" community-id="' + data.id +  '"  onclick="approveCommunity($(this))">Approve</button>';
                            element += '</div>';
                            element += '<div class="col-xl-6" style="text-align: center">';
                            element += '<button type="button" class="mb-1 mt-1 mr-1 btn btn-danger" community-id="' + data.id +  '" onclick="declineCommunity($(this))">Decline</button>';
                            element += '</div></div>'
                            return element;
                        }
                    }
	...
	$(function() {
		UserListTable.initialize();
        ReporterListTable.initialize();
        CommunityListTable.initialize();
        CommunitySubmitListTable.initialize();
	});
